...
from web_search import bing_is_available, collect
...
class DeepDeepBrain:
    ...
    def _build_web_messages(self, user_id: str, conversation_id: str, query: str) -> List[Dict[str, str]]:
        if not self.web_enabled:
            raise RuntimeError("Bing is not reachable, so web search is disabled.")
    ...
    def generate(self, user_id: str, conversation_id: str, user_message: str, web: bool = False) -> str:
        if self.model is None or self.tokenizer is None:
            raise RuntimeError("The model is not loaded")
        messages = (
            self._build_web_messages(user_id, conversation_id, user_message)
            if web
            else self._build_messages(user_id, conversation_id, user_message)
        )
    ...
    def chat(self, user_id: str, conversation_id: str, user_message: str) -> str:
        ...
        web = user_message.startswith("/web ")
        prompt = user_message[5:].strip() if web else user_message
        if not prompt:
            raise ValueError("Use /web followed by a search query.")
        reply = self.generate(user_id, conversation_id, prompt, web=web)
